...
Extend `$continue-task` so an in-progress primary task can add a new dependency
...
- [x] `$continue-task` can propose a new `dependency_step` for the selected primary task when execution discovers a missing dependency.
...
- 20. Continue-task can propose new dependency steps mid-execution
