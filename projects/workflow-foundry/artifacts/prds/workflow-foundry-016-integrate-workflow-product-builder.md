...
Use the existing tracker-maintenance authority to add only the minimum v1
deliverable-migration metadata required for preserved newer open tasks. Freeze
their current phase and authority. If live evidence disproves a saved phase,
correct it conservatively: Health task 007 claimed code-review readiness while
its standalone product remained uncommitted and had no durable review artifact,
so return it to implement with a frozen pending migration rather than inventing
completion evidence or binding public state to a private machine path. These
compatibility writes do not advance downstream work and are part of integration
correctness.
...
10. As the Health workflow owner, I want Health task 008 to resolve 015 as an
    active completed capability, so that the foundry dependency is no longer the
    reason Health UI planning is blocked.
...
- Do not alter downstream task phase, dependency write plans, approved artifacts,
  or product authority. Tracker compatibility may add frozen migration metadata
  under the existing tracker-maintenance owner; downstream readiness checks remain
  read-only beyond that schema reconciliation.
...
5. **Capability dependency resolution:** Health task 008 resolves the selected
   canonical skill and completed 015 dependency while remaining in its own saved
   phase with no new write authority. Its overall task readiness may remain
   blocked by its own pending migration and phase decisions.
...
- If a durable testing surface accepted private content, exclude the private
  record from publication and repair the write and validation boundary with a
  regression test before adoption.
...
- Advancing downstream project tasks, changing their product decisions, or
  granting new write authority; minimal frozen tracker-schema migration is
  explicitly in scope.
